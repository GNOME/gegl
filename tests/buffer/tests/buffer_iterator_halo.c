...
#define SIZE_DIFF     42
#define WIDTH_NEED    ((KERNEL_WIDTH  - 1) / 2)
#define HEIGHT_NEED   ((KERNEL_HEIGHT - 1) / 2)
...
TEST ()
{
  ...
  GeglRectangle       in_roi     = {
    -SIZE_DIFF,
    -SIZE_DIFF,
    out_roi.width + SIZE_DIFF * 2,
    out_roi.height + SIZE_DIFF * 2,
  };
  gint                components = 0;

  test_start();

  g_assert_cmpint (KERNEL_WIDTH, <=, SIZE_DIFF);
  g_assert_cmpint (KERNEL_HEIGHT, <=, SIZE_DIFF);

  out_buffer = gegl_buffer_new (&out_roi, babl_format ("Y float"));
  /* Uses out_roi on purpose. Relies on the abyss for the out-of-bounds halo
   * values. */
  ...
  while (gegl_buffer_iterator_next (iter))
    {
      ...
      g_assert_false (gegl_rectangle_equal (&iter->items[1].roi,
                                            &iter->items[0].roi));
      g_assert_true (iter->items[1].roi.width - iter->items[0].roi.width ==
                     in_roi.width - out_roi.width);
      g_assert_true (iter->items[1].roi.height - iter->items[0].roi.height ==
                     in_roi.height - out_roi.height);
      ...
      for (gint i = 0; i < iter->length; i++)
        {
          gfloat val       = 0;
          gint   in_offset = (iter->items[1].roi.width * (SIZE_DIFF - WIDTH_NEED))       +
                             (SIZE_DIFF - HEIGHT_NEED)                                   +
                             (iter->items[1].roi.width * (i / iter->items[0].roi.width)) +
                             i % iter->items[0].roi.width;

          for (int y = 0; y < KERNEL_HEIGHT; y++)
            {
              gint idx_y    = y * KERNEL_WIDTH;
              gint offset_y = y * iter->items[1].roi.width * components;

              for (int x = 0; x < KERNEL_WIDTH; x++)
                {
                  kernel[idx_y + x] = in[in_offset + offset_y + x * components];
                }
            }
