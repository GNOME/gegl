...
load_incomplete (void)
{
  ...
  if (! gegl_node_process_success (sink, &error))
    {
      if (error                                                &&
          error->domain == g_quark_from_static_string ("gegl") &&
          error->code == 0)
        status = SUCCESS;
    }
  MORE_INFO ("[gegl:buffer-sink] Operation gegl:buffer-sink failed: processing 0px rectangle.",
             g_quark_from_static_string ("gegl"), 0);
