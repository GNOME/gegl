...
#define GEGL_OP_SINK
#define GEGL_OP_NAME sdl3_display
#define GEGL_OP_C_SOURCE sdl3-display.c
  ...
  if (!state->window ||
       ...
       state->height != result->height)
    {
      ...
      if (state->window)
        ...
        {
          if (!SDL_CreateWindowAndRenderer (o->window_title, result->width,
                  result->height, 0,
      ...
      SDL_DestroySurface (state->screen);
      state->screen = SDL_CreateSurface (
              result->width, result->height, SDL_PIXELFORMAT_RGBA8888);
      if (!state->screen)
        {
...
finalize (GObject *object)
{
  ...
  if (o->user_data)
    {
      state = (SDLState *)o->user_data;
      if (state->texture) SDL_DestroyTexture (state->texture);
      if (state->screen) SDL_DestroySurface (state->screen);
...
gegl_op_class_init (GeglOpClass *klass)
{
  ...
  gegl_operation_class_set_keys (operation_class,
    "name",         "gegl:sdl3-display",
    "title",        _("SDL3 Display"),
    "categories",   "display",
    "description",
        _("Displays the input buffer in an SDL3 window."),
        NULL);
}
#endif
