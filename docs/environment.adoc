...
[[GEGL_OPENCL_PROFILING]]
GEGL_OPENCL_PROFILING::
  [`true`, `false`] default: `false` +
  Create OpenCL command queues with the `CL_QUEUE_PROFILING_ENABLE` flag
  enabled. This makes it possible to profile individual OpenCL commands.
  `1` and `yes` are synonyms for `true`, everything else is taken as
  `false`.
