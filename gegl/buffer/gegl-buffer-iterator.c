  ...
  for (gint index = 1; index < priv->used_slots; index++)
    {
      ...
      sub->current_roi.width  = MAX(1, lead_sub->current_roi.width  + roi_offset_width);
      sub->current_roi.height = MAX(1, lead_sub->current_roi.height + roi_offset_height);
...
static gint
sort_sub_iterator_access_order (gconstpointer a,
                    gconstpointer b,
                    gpointer      user_data)
{
...
prepare_iterator (GeglBufferIterator *iter)
{
  ...
  g_sort_array (access_order, priv->used_slots, sizeof (gint), sort_sub_iterator_access_order, priv);
#else
  g_qsort_with_data (access_order, priv->used_slots, sizeof (gint), sort_sub_iterator_access_order, priv);
#endif
  ...
  if (next_state == GeglIteratorState_InRows)
    {
      ...
      // Since we're going by rows, the first iterator's height is always 1 but
      // the others might have a halo around it. In case of an ROI smaller than
      // of the first sub-iterator, the height of that region is also set to 1.
      iter->items[0].roi.height = 1;
      for (gint index = 1; index < priv->used_slots; index++)
        {
          SubIterState *sub           = &priv->sub_iter[index];
          int           offset_height = MAX(0, sub->full_roi.height - priv->sub_iter[0].full_roi.height);
