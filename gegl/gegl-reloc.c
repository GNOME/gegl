 ...
 */
static char *
_br_find_exe (GeglBinrelocInitError *error)
{
#if ! defined(ENABLE_RELOCATABLE_RESOURCES) || defined(G_OS_WIN32) || defined(__APPLE__)
  if (error)
    *error = GEGL_RELOC_INIT_ERROR_DISABLED;
  return NULL;
#else
  ...
  if (! input)
    {
      ...
      if (error)
        *error = GEGL_RELOC_INIT_ERROR_OPEN_MAPS;
      ...
      if (maps_line == NULL)
        {
          ...
          if (error)
            *error = GEGL_RELOC_INIT_ERROR_READ_MAPS;
  ...
  if (path == NULL && error)
    *error = GEGL_RELOC_INIT_ERROR_INVALID_MAPS;
 ...
 */
static char *
_br_find_exe_for_symbol (const void *symbol, GeglBinrelocInitError *error)
{
#if ! defined(ENABLE_RELOCATABLE_RESOURCES) || defined(G_OS_WIN32) || defined(__APPLE__)
  if (error)
    *error = GEGL_RELOC_INIT_ERROR_DISABLED;
...
static void set_gerror (GError **error, GeglBinrelocInitError errcode);
 ...
 *               error report. See the #GeglBinrelocInitError for a list of error
 *               codes.
 *
 * @returns TRUE on success, FALSE if BinReloc failed to initialize.
 */
gboolean
_gegl_reloc_init (GError **error)
{
  GeglBinrelocInitError errcode;

  /* Shut up compiler warning about uninitialized variable. */
  errcode = GEGL_RELOC_INIT_ERROR_NOMEM;
 ...
 */
gboolean
_gegl_reloc_init_lib (GError **error)
{
  GeglBinrelocInitError errcode;
...
static void
set_gerror (GError **error, GeglBinrelocInitError errcode)
{
 ...
 */
gchar *
_gegl_reloc_find_prefix (const gchar *default_prefix)
{
  gchar *dir1, *dir2;
  gchar *exe_dir;
